...
class GetWorkTest(unittest.TestCase):
    ...
    def testWorkAliases(self):
        res = _common.open_and_parse_test_data(self.datadir, "80737426-8ef3-3a9c-a3a6-9507afb93e93_inc=aliases.xml")
        aliases = res["work"]["alias-list"]
        self.assertEqual(len(aliases), 3)

        a1 = aliases[1]
        self.assertEqual(a1["alias"], 'Symphonie Nr. 3 Es-Dur, Op. 55 "Eroica"')
        self.assertEqual(a1["sort-name"], 'Symphonie Nr. 3 Es-Dur, Op. 55 "Eroica"')

        a2 = aliases[2]
        self.assertEqual(a2["alias"], 'Symphony No. 3, Op. 55 "Eroica"')
        self.assertEqual(a2["sort-name"], 'Symphony No. 3, Op. 55 "Eroica"')

        res = _common.open_and_parse_test_data(self.datadir, "3d7c7cd2-da79-37f4-98b8-ccfb1a4ac6c4_inc=aliases.xml")
        aliases = res["work"]["alias-list"]
    ...
    def testWorkAttributes(self):
        ...
        res = _common.open_and_parse_test_data(self.datadir, "8e134b32-99b8-4e96-ae5c-426f3be85f4c_attributes.xml")
        work_attrs = res["work"]["attribute-list"]
    ...
    def testWorkRelationAttributes(self):
        ...
        res = _common.open_and_parse_test_data(self.datadir, "72c9aad2-3c95-4e3e-8a01-3974f8fef8eb_inc=series-rels.xml")
